...
srun --nodes="${NNODES}" --ntasks="${NNODES}" --ntasks-per-node=1 bash -lc '
  ...
  export CHECKPOINT_RANK0_TIMEOUT_S="'"$CHECKPOINT_RANK0_TIMEOUT_S"'"
  export VALIDATION_RANK0_TIMEOUT_S="'"$VALIDATION_RANK0_TIMEOUT_S"'"
  export RANK0_SECTION_POLL_INTERVAL_S="'"$RANK0_SECTION_POLL_INTERVAL_S"'"
  export REWARD_WEIGHTS="'"$REWARD_WEIGHTS"'"
