#!/usr/bin/env python3
"""Build CAFA-style information accretion weights.

Input annotations must already be ancestor-propagated. The expected TSV columns
are the temporal split artifact columns: DB_ID, GO_ID, Aspect.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--annotations", required=True, help="Propagated training annotations TSV.")
...
def load_go_metadata(obo_path: Path) -> Dict[str, Dict[str, object]]:
    ...
    def finalize_term() -> None:
        ...
        if current_id and current_namespace and not current_obsolete:
            parents: List[str] = []
            seen: Set[str] = set()
...
def count_parent_context_support(
    ...
    for go_id, metadata in go_metadata.items():
        ...
        counts[go_id] = sum(
            1 for assigned_terms in protein_to_terms.values() if parent_set.issubset(assigned_terms)
        )
    return counts
    ...
    *,
    ...
    for go_id, metadata in go_metadata.items():
        ...
        if term_count <= 0:
            continue
        parents = tuple(metadata.get("parents", ()))
        context_count = float(parent_support.get(go_id, 0)) if parents else float(total_proteins)
        if context_count <= 0:
            continue
        probability = min(max(term_count / context_count, 1e-12), 1.0)
        ia_value = -math.log(probability) / log_denominator
        if ia_value > 0.0:
