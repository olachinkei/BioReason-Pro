...
IS_SWISSPROT=False
VALIDATION_SUBSET_SIZE=${VALIDATION_SUBSET_SIZE:-200}
...
BENCHMARK_VERSION="train=213->225, future=225->228, dev=200, holdout=400"
...
if as_bool "$RUN_STAGE1"; then
  ...
  if [ "${STAGE1_CHECKPOINT_ARTIFACT_NAME+x}" = "x" ]; then
    STAGE1_CHECKPOINT_ARTIFACT_NAME="${STAGE1_CHECKPOINT_ARTIFACT_NAME}"
  elif [ "${CHECKPOINT_ARTIFACT_NAME+x}" = "x" ]; then
    STAGE1_CHECKPOINT_ARTIFACT_NAME="${CHECKPOINT_ARTIFACT_NAME}"
  else
    STAGE1_CHECKPOINT_ARTIFACT_NAME="${WANDB_RUN_NAME_S1}-checkpoints"
  fi
...
if [ -z "${RUN_NAME_S2_DIR:-}" ]; then
  if [ "${WANDB_RUN_NAME_S2+x}" = "x" ] || [ "${WANDB_RUN_NAME+x}" = "x" ]; then
    RUN_NAME_S2_DIR="${WANDB_RUN_NAME_S2}"
  else
    RUN_NAME_S2_DIR="${RUN_NAME_S2_DIR_DEFAULT}"
  fi
fi
STAGE2_CHECKPOINT_DIR="${STAGE2_CHECKPOINT_DIR:-${BASE_CHECKPOINT_DIR}/${RUN_NAME_S2_DIR}}"
STAGE2_MODEL_ARTIFACT_NAME="${STAGE2_MODEL_ARTIFACT_NAME:-train-sft-output}"
if [ "${STAGE2_CHECKPOINT_ARTIFACT_NAME+x}" = "x" ]; then
  STAGE2_CHECKPOINT_ARTIFACT_NAME="${STAGE2_CHECKPOINT_ARTIFACT_NAME}"
elif [ "${CHECKPOINT_ARTIFACT_NAME+x}" = "x" ]; then
  STAGE2_CHECKPOINT_ARTIFACT_NAME="${CHECKPOINT_ARTIFACT_NAME}"
else
  STAGE2_CHECKPOINT_ARTIFACT_NAME="${WANDB_RUN_NAME_S2}-checkpoints"
fi
...
stdbuf -oL -eL "${BASE_COMMAND[@]}" \
    ...
    --model_artifact "${STAGE2_MODEL_ARTIFACT_NAME}" \
    ...
    --every_n_train_steps "$STAGE2_CHECKPOINT_EVERY_N_TRAIN_STEPS" \
    --sample_generation_every_n_steps "$STAGE2_SAMPLE_GENERATION_EVERY_N_STEPS" \
