...
def require_torch() -> None:
    if torch is None or F is None:
        raise RuntimeError("train.py backend training requires torch to be installed.")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--backend_train", action="store_true", help=argparse.SUPPRESS)
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.environ.get("CHECKPOINT_EXPORT_MODE", ""),
        choices=["", "full", "adapter_only", "inference_export"],
        help="Checkpoint payload mode. inference_export skips DeepSpeed state and writes an eval-ready export.",
    )
    ...
    parser.add_argument("--wandb_group", type=str, default=os.environ.get("WANDB_GROUP", ""))
...
def initialize_runtime(args: argparse.Namespace) -> DistributedRuntime:
    ...
    emit_debug_log(
        run_id=run_id,
        hypothesis_id="H4",
        location="train.py:initialize_runtime:pre_init",
    ...
    emit_debug_log(
        run_id=run_id,
        hypothesis_id="H4",
        location="train.py:initialize_runtime:post_init",
    ...
    if runtime.enabled:
        ...
        try:
            ...
            emit_debug_log(
                run_id=normalize_text(os.environ.get("SLURM_JOB_ID")).strip() or "local",
                hypothesis_id="H1",
                location="train.py:all_reduce_sum_scalar",
    ...
    if runtime.enabled:
        ...
        try:
            ...
            emit_debug_log(
                run_id=normalize_text(os.environ.get("SLURM_JOB_ID")).strip() or "local",
                hypothesis_id="H1",
                location="train.py:all_reduce_max_scalar",
...
class RunTracker:
    ...
    def _maybe_init_wandb(self, config: Mapping[str, Any]) -> Any:
        ...
        if explicit_wandb_resume in {"1", "true", "t", "yes", "y"}:
            init_kwargs["resume"] = True
        wandb_group = normalize_text(getattr(self.args, "wandb_group", "")).strip() or normalize_text(
            os.environ.get("WANDB_GROUP")
        ).strip()
        if wandb_group:
...
def save_training_checkpoint(
    ...
    checkpoint_export_mode = normalize_text(getattr(args, "checkpoint_export_mode", "")).strip() or (
        "adapter_only" if checkpoint_export_only else "full"
    )
    ...
    def export_checkpoint_artifact() -> None:
        ...
        metadata = {
            ...
            "checkpoint_export_only": checkpoint_export_mode != "full",
        ...
        if checkpoint_export_mode in {"full", "inference_export"}:
    ...
    if checkpoint_export_mode in {"adapter_only", "inference_export"}:
        rank0_print(runtime, f"Saving {checkpoint_export_mode} checkpoint artifact for step {step}.")
...
def train(args: argparse.Namespace) -> None:
    ...
    def _iterate_steps_with_weave_span() -> Iterable[int]:
        resume_start_step = max(int(getattr(args, "resume_start_step", 0) or 0), 0)
        for _step in range(resume_start_step, resume_start_step + int(args.max_steps)):
        ...
        for step in _iterate_steps_with_weave_span():
            ...
            if runtime.rank == 0:
                ...
                emit_debug_log(
                    run_id=normalize_text(os.environ.get("SLURM_JOB_ID")).strip() or "local",
                    hypothesis_id="H3",
                    location="train.py:train:step_start",
            ...
            if runtime.rank == 0:
                ...
                emit_debug_log(
                    run_id=normalize_text(os.environ.get("SLURM_JOB_ID")).strip() or "local",
                    hypothesis_id="H2",
                    location="train.py:train:pre_refresh",
            ...
            if runtime.rank == 0:
                ...
                emit_debug_log(
                    run_id=normalize_text(os.environ.get("SLURM_JOB_ID")).strip() or "local",
                    hypothesis_id="H2",
                    location="train.py:train:post_refresh_barrier",
...
def backend_main(argv: Optional[Sequence[str]] = None) -> None:
...
def build_senpai_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Senpai entrypoint for BioReason-Pro. Runs baseline validation, "
            "a 5-step gate, and optional continuation when validation improves."
        )
    )
    parser.add_argument("--mode", choices=["auto", "baseline", "gate"], default="auto")
    parser.add_argument("--wandb_name", "--wandb-name", dest="wandb_name", type=str, default="senpai/bioreason")
    parser.add_argument("--wandb_group", "--wandb-group", dest="wandb_group", type=str, default="")
    parser.add_argument("--wandb_entity", "--wandb-entity", dest="wandb_entity", type=str, default=os.environ.get("WANDB_ENTITY", ""))
    parser.add_argument(
        "--wandb_project",
        "--wandb-project",
        dest="wandb_project",
    ...
    parser.add_argument("--primary_metric", "--primary-metric", dest="primary_metric", type=str, default="overall_mean_fmax")
    ...
    parser.add_argument("--base_checkpoint", "--base-checkpoint", dest="base_checkpoint", type=str, default=os.environ.get("BIOREASON_RL_PAPER_MODEL_REGISTRY_PATH", ""))
    parser.add_argument("--baseline_metrics_path", "--baseline-metrics-path", dest="baseline_metrics_path", type=str, default="")
    parser.add_argument("--runtime_root", "--runtime-root", dest="runtime_root", type=str, default=os.environ.get("BIOREASON_RUNTIME_ROOT", ""))
    parser.add_argument("--output_dir", "--output-dir", dest="output_dir", type=str, default="")
    parser.add_argument("--python_bin", "--python-bin", dest="python_bin", type=str, default=sys.executable)
    parser.add_argument("--deepspeed_bin", "--deepspeed-bin", dest="deepspeed_bin", type=str, default=os.environ.get("DEEPSPEED_BIN", "deepspeed"))
    parser.add_argument("--nnodes", type=int, default=int(os.environ.get("NNODES", "1")))
    parser.add_argument("--gpus_per_node", "--gpus-per-node", dest="gpus_per_node", type=int, default=int(os.environ.get("GPUS_PER_NODE", "8")))
    parser.add_argument("--master_addr", "--master-addr", dest="master_addr", type=str, default=os.environ.get("MASTER_ADDR", "127.0.0.1"))
    parser.add_argument("--master_port", "--master-port", dest="master_port", type=str, default=os.environ.get("MASTER_PORT", "29500"))
    parser.add_argument("--dry_run", "--dry-run", dest="dry_run", action="store_true")
    return parser


def parse_senpai_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = build_senpai_arg_parser()
    args, backend_args = parser.parse_known_args(argv)
    args.backend_args = list(backend_args)
    return args


def default_senpai_runtime_root() -> Path:
    configured = normalize_text(os.environ.get("BIOREASON_RUNTIME_ROOT")).strip()
    if configured:
        return Path(configured).expanduser().resolve()
    user = normalize_text(os.environ.get("USER")).strip()
    if user and Path("/mnt/data").exists():
        return (Path("/mnt/data") / user / "BioReason-Pro").resolve()
    return (Path.cwd() / "data" / "artifacts" / "senpai_runtime").resolve()


def sanitize_run_name(value: str) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]+", "-", normalize_text(value).strip())
    return safe.strip("-") or "senpai-bioreason"


def configure_senpai_runtime_env(args: argparse.Namespace) -> Dict[str, str]:
    runtime_root = Path(args.runtime_root).expanduser().resolve() if normalize_text(args.runtime_root).strip() else default_senpai_runtime_root()
    artifacts_root = runtime_root / "data" / "artifacts"
    cache_root = runtime_root / "cache"
    env = os.environ.copy()
    env.setdefault("BIOREASON_RUNTIME_ROOT", str(runtime_root))
    env.setdefault("BIOREASON_ARTIFACTS_ROOT", str(artifacts_root))
    env.setdefault("BIOREASON_CACHE_ROOT", str(cache_root))
    env.setdefault("WANDB_DIR", str(runtime_root / "wandb"))
    env.setdefault("WEAVE_SERVER_CACHE_DIR", str(runtime_root / "wandb" / "weave_server_cache"))
...
def maybe_load_registry_env(args: argparse.Namespace) -> None:
    registry_env = normalize_text(args.registry_env_file).strip()
    if not registry_env:
        return
    path = Path(registry_env)
    if not path.is_absolute():
        path = Path.cwd() / path
    if not path.exists():
        return
    from bioreason2.utils.research_registry import load_exported_env_file

    load_exported_env_file(str(path), override=False)


def run_capture(command: Sequence[str], env: Mapping[str, str]) -> str:
...
def resolve_senpai_assets(args: argparse.Namespace, env: Mapping[str, str]) -> Dict[str, str]:
    ...
    resolver_env.update(os.environ)
    base_checkpoint = normalize_text(args.base_checkpoint).strip() or normalize_text(
        os.environ.get("BIOREASON_RL_PAPER_MODEL_REGISTRY_PATH")
    ).strip()
    if not base_checkpoint:
        raise RuntimeError("Set BIOREASON_RL_PAPER_MODEL_REGISTRY_PATH or pass --base_checkpoint.")

    artifacts_root = Path(env["BIOREASON_ARTIFACTS_ROOT"])
    model_dir = artifacts_root / "models" / "bioreason_pro_rl_paper"
    model_dir.mkdir(parents=True, exist_ok=True)
    resolved_model_dir = run_capture(
        [
            args.python_bin,
            "scripts/materialize_model_source.py",
            "--wandb-registry-path",
            base_checkpoint,
            "--local-dir",
            str(model_dir),
    ...
    def bundle_field(asset_key: str, field: str) -> str:
        return run_capture(
            [
                args.python_bin,
                "scripts/materialize_data_bundle.py",
                "--data-manifest-path",
                args.data_manifest_path,
                "--data-bundle",
                args.data_bundle,
                "--asset-key",
                asset_key,
    ...
    dataset_dir = bundle_field("reasoning_dataset", "local_dir")
    dataset_name = bundle_field("reasoning_dataset", "dataset_name")
    dataset_artifact = bundle_field("reasoning_dataset", "wandb_registry_path")
    temporal_split_artifact = bundle_field("temporal_split_artifact", "wandb_registry_path")
    ia_dir = bundle_field("ia_file", "local_dir")
    ia_file = str(Path(ia_dir) / "IA.txt")
    return {
...
def read_metric(metrics: Mapping[str, Any], key: str) -> Optional[float]:
...
def find_metrics_summary(output_dir: Path) -> Path:
    candidates = [
        output_dir / "cafa_metrics" / "metrics_summary.json",
        output_dir / "results" / "cafa_metrics" / "metrics_summary.json",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    matches = sorted(output_dir.glob("**/metrics_summary.json"))
    if matches:
        return matches[0]
    return candidates[0]
...
def run_eval_phase(
    *,
    ...
    eval_env.update(
        {
            ...
            "MAX_SAMPLES": str(int(args.max_val_samples)),
            "MAX_NEW_TOKENS": str(int(args.eval_max_new_tokens)),
            "SAMPLE_STRATEGY": "stratified_aspect_profile",
            "EVALS_PATH": str(output_dir),
            "CAFA5_DATASET": assets["dataset_dir"],
            "DATASET_NAME": assets["dataset_name"],
            "REASONING_DATASET_NAME": assets["dataset_name"],
            "DATASET_ARTIFACT": assets["dataset_artifact"],
            "TEMPORAL_SPLIT_ARTIFACT": assets["temporal_split_artifact"],
            "IA_FILE_PATH": assets["ia_file"],
            "GO_OBO_PATH": "bioreason2/dataset/go-basic.obo",
            "WANDB_ENTITY": normalize_text(args.wandb_entity).strip(),
            "WANDB_PROJECT": normalize_text(args.wandb_project).strip(),
            "WANDB_MODE": normalize_text(args.wandb_mode).strip(),
            "WANDB_RUN_NAME": run_name,
            "WANDB_ARTIFACT_NAME": f"{sanitize_run_name(run_name)}-results",
    ...
    if args.dry_run:
        metrics_path = output_dir / "cafa_metrics" / "metrics_summary.json"
...
def read_wandb_run_id(output_dir: Path) -> Optional[str]:
    info_path = output_dir / "wandb_run_info.json"
    ...
    try:
        ...
        return None
    run_id = normalize_text(payload.get("run_id")).strip() if isinstance(payload, Mapping) else ""
    return run_id or None


def build_backend_train_command(
    *,
    args: argparse.Namespace,
    assets: Mapping[str, str],
    phase: str,
    ...
    command = [
        ...
        str(int(args.nnodes)),
        "--num_gpus",
        str(int(args.gpus_per_node)),
        "--master_addr",
        normalize_text(args.master_addr).strip() or "127.0.0.1",
        "--master_port",
        normalize_text(args.master_port).strip() or "29500",
        "train.py",
        "--backend_train",
        "--text_model_name",
        assets["model_dir"],
        ...
        assets["temporal_split_artifact"],
        "--dataset_artifact",
        assets["dataset_artifact"],
        "--go_obo_path",
        ...
        "0",
        "--save_every_n_steps",
        str(int(save_every_n_steps)),
        "--checkpoint_export_mode",
        "inference_export",
        "--output_dir",
        str(output_dir),
        "--checkpoint_artifact_name",
        f"train-rl-output-{phase}",
        "--checkpoint_artifact_aliases",
        f"latest,{phase}",
        "--run_name",
        f"{normalize_text(args.wandb_name).strip()}-{phase}",
        "--wandb_project",
        normalize_text(args.wandb_project).strip(),
        "--wandb_group",
        normalize_text(args.wandb_group).strip(),
        "--queries_per_step",
        ...
        "--weave_trace_budget",
        os.environ.get("SENPAI_WEAVE_TRACE_BUDGET", "16"),
    ]
    if normalize_text(args.wandb_entity).strip():
        command.extend(["--wandb_entity", normalize_text(args.wandb_entity).strip()])
    if normalize_text(args.wandb_mode).strip():
        command.extend(["--wandb_mode", normalize_text(args.wandb_mode).strip()])
    if normalize_text(args.weave_project).strip():
        command.extend(["--weave_project", normalize_text(args.weave_project).strip()])
    if resume_from_checkpoint is not None:
        command.extend(["--resume_from_export_artifact", str(resume_from_checkpoint), "--resume_mode", "warm"])
    command.extend(list(getattr(args, "backend_args", [])))
    return command
...
def run_train_phase(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    command = build_backend_train_command(
        args=args,
        assets=assets,
        phase=phase,
        max_steps=max_steps,
    ...
    train_env.update(
        {
            "WANDB_GROUP": normalize_text(args.wandb_group).strip(),
    ...
    if args.dry_run:
        step = int(save_every_n_steps)
        checkpoint_root = output_dir / "checkpoints" / f"step-{step:06d}"
        export_dir = checkpoint_root / "inference_export"
        export_dir.mkdir(parents=True, exist_ok=True)
        save_json(checkpoint_root / "training_metadata.json", {"global_step": step, "dry_run": True})
    else:
        subprocess.run(command, cwd=Path.cwd(), env=train_env, check=True)
    checkpoint_root = output_dir / "checkpoints" / f"step-{int(save_every_n_steps):06d}"
    return checkpoint_root, read_wandb_run_id(output_dir)


def emit_senpai_result(
    *,
    terminal: bool,
    status: str,
    ...
    payload = {
        ...
        "status": status,
        "pending_arms": False,
        "wandb_run_ids": [run_id for run_id in run_ids if run_id],
        "primary_metric": {"name": primary_metric_name, "value": float(primary_metric_value)},
        "test_metric": {"name": primary_metric_name, "value": float(test_metric_value)},
    }
    print(f"SENPAI-RESULT: {json.dumps(payload, sort_keys=True, separators=(',', ':'))}", flush=True)
...
def senpai_main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    env = configure_senpai_runtime_env(args)
    assets = resolve_senpai_assets(args, env)
    run_slug = sanitize_run_name(args.wandb_name)
    run_root = (
        Path(args.output_dir).expanduser().resolve()
        if normalize_text(args.output_dir).strip()
        else Path(env["BIOREASON_ARTIFACTS_ROOT"]) / "senpai_runs" / f"{run_slug}-{int(time.time())}"
    )
    run_root.mkdir(parents=True, exist_ok=True)

    baseline_metrics_path = (
        Path(args.baseline_metrics_path).expanduser().resolve()
        if normalize_text(args.baseline_metrics_path).strip()
        else run_root / "baseline" / "cafa_metrics" / "metrics_summary.json"
    )
    if baseline_metrics_path.exists():
        baseline_metrics = load_metrics_file(baseline_metrics_path)
    else:
        baseline_metrics, baseline_metrics_path = run_eval_phase(
            args=args,
            env=env,
            assets=assets,
            model_path=Path(assets["model_dir"]),
            output_dir=run_root / "baseline",
            run_name=f"{args.wandb_name}-baseline",
            model_name="bioreason-pro-rl-paper",
        )
    baseline_value = read_metric(baseline_metrics, args.primary_metric)
    if baseline_value is None:
        raise RuntimeError(f"Baseline metric {args.primary_metric!r} is missing or non-finite in {baseline_metrics_path}.")

    if args.mode == "baseline":
        save_json(run_root / "senpai_summary.json", {"baseline_metrics": baseline_metrics, "baseline_metrics_path": str(baseline_metrics_path)})
        emit_senpai_result(
            terminal=True,
            status="complete",
            run_ids=[],
            primary_metric_name=args.primary_metric,
            primary_metric_value=baseline_value,
            test_metric_value=baseline_value,
        )
        return

    gate_output_dir = run_root / "train-gate"
    gate_checkpoint, gate_run_id = run_train_phase(
        args=args,
        env=env,
        assets=assets,
        phase="gate",
        max_steps=int(args.gate_steps),
    ...
    if args.mode == "gate" or not improved or int(args.continue_steps) <= 0:
        ...
        reported_value = gate_value if gate_value is not None else 0.0
        emit_senpai_result(
            terminal=True,
            status="complete",
            run_ids=run_ids,
            primary_metric_name=args.primary_metric,
            primary_metric_value=reported_value,
            test_metric_value=reported_value,
        )
        return

    total_steps = int(args.gate_steps) + int(args.continue_steps)
    continue_output_dir = run_root / "train-continue"
    continue_checkpoint, continue_run_id = run_train_phase(
        args=args,
        env=env,
        assets=assets,
        phase="continue",
        max_steps=int(args.continue_steps),
        save_every_n_steps=total_steps,
        output_dir=continue_output_dir,
        resume_from_checkpoint=gate_checkpoint,
    )
    if continue_run_id:
    ...
    summary.update(
        {
            "continue_checkpoint": str(continue_checkpoint),
            "continue_metrics_path": str(continue_metrics_path),
    ...
    emit_senpai_result(
        ...
        test_metric_value=best_value,
    )


def main(argv: Optional[Sequence[str]] = None) -> None:
    raw_args = list(sys.argv[1:] if argv is None else argv)
    if "--backend_train" in raw_args:
        backend_main(raw_args)
        return
    senpai_main(raw_args)
